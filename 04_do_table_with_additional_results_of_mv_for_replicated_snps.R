...
names_mv <- c("multi_N_glycosylation", "multi_bisectingGlcNAc")

snps <- matrix(ncol=2, nrow=5) 


snps[,1] <- c("rs11895615", "rs1372288", "rs12635457", "rs479844", "rs4561508")
snps[,2] <- c("multi_bisectingGlcNAc", "multi_N_glycosylation", "multi_N_glycosylation", "multi_N_glycosylation", "multi_N_glycosylation")
	...
	rownames(coef) <- MV_P$scan[,1]
		...
		for (i in (1:nrow(snps))) {
			...
			if (x==snps$trait[i]) {
				...
				extracted_data_coef[[i]] <- coef[(which(snps$SNP[i]==rownames(coef))),-c(2:6)]
